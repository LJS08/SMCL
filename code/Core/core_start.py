...
def _downloads_hash_bugs(link_downloads_assets, Max_try, file_dir, downloads_file_url_where, back_dir, file_path_name,
    ...
    """
    temp_link_downloads_assets是像http://img.download.minecraft.net/的。
    Max_try 是最大尝试次数。
...
def core_bootstrap_main(selfup, mc_path, jar_version, link_type):
    ...
    if selfup:
        ...
        if os.path.isfile(os.path.join(mc_path, "versions", jar_version, jar_version) + ".jar"):
            print("主游戏文件:存在")
            logger.info(
                "主游戏文件:存在,{}".format((os.path.join(mc_path, "versions", jar_version, jar_version) + ".jar")))
        else:
            print("主游戏文件:不存在")
            logger.info(
                "主游戏文件:不存在,{}".format((os.path.join(mc_path, "versions", jar_version, jar_version) + ".jar")))
        ...
        if file_index_objects:  # 如果file_index_objects的值为真（1.12.json存在的情况）
            ...
            while hash_yn == 0:
                ...
                if hash_yn_too_many_try >= 10:  # 尝试次数过多就引发这个错误,防止程序一直卡在这
                    raise CoreBootstrapMainError(
                        "验证资源索引文件时尝试次数过多")  # 可以考虑在参数上加上循环次数过多时候是否退出（core_bootstrap_main)
            ...
            for asserts_json_objects_hash_name in asserts_json_objects_hash:
                if os.path.isdir(os.path.join(assets_objects_path, asserts_json_objects_hash_name[0:2])):  # 资源文件夹是否存在
                    if os.path.isfile(os.path.join(assets_objects_path, asserts_json_objects_hash_name[0:2],
                        ...
                        downloads_file_url_list.append({"url": (link_downloads_assets + asserts_json_objects_hash_name[
                                                                                        0:2] + "/" + asserts_json_objects_hash_name),
                                                        "path": (os.path.join(assets_objects_path,
                                                                              asserts_json_objects_hash_name[0:2],
                                                                              asserts_json_objects_hash_name)),
...
def core_start_Login(Update_Token, Refresh_Token, Mojang_MS_login=False, MS_login=False, back_url: str = None,
                     Mojang_login=False):
    """
    ...
    if version_forge == "latest":
        ...
        if not VT_bit:
            ...
            if not os.path.exists(
                    "forge-{0}-{1}-installer.jar".format(version_game, forge_downloads_clint_install_version)):
                os.chdir(running_path)
                logger.debug("forge安装包名为:forge-{0}-{1}-installer.jar".format(version_game,
                                                                                  forge_downloads_clint_install_version))

                _downloads_file_url("https://bmclapi2.bangbang93.com/forge/download/{}".format(max(forge_build_list)), (
                    os.path.join(game_path, "forge-{0}-{1}-installer.jar".format(version_game,
                                                                                 forge_downloads_clint_install_version))),
                                    True)

                logger.debug("forge安装目录为:{}".format((os.path.join(mc_path, "forge-{0}-{1}-installer.jar".format(
                    version_game, forge_downloads_clint_install_version)))))
            else:
                logger.info("已检测到forge安装包，正在验证是否可用")
                if _hash_get_val(
                        ("forge-{0}-{1}-installer.jar".format(version_game, forge_downloads_clint_install_version)),
                        "sha1") == forge_downloads_clint_install_hash:
                    logger.info("此安装包可用")
                    os.chdir(running_path)
                else:
                    xhkz = True
                    while xhkz:
                        ...
                        _downloads_file_url(
                            "https://bmclapi2.bangbang93.com/forge/download/{}".format(max(forge_build_list)), (
                                os.path.join(game_path, "forge-{0}-{1}-installer.jar".format(version_game,
                                                                                             forge_downloads_clint_install_version))),
                            True)
                        if _hash_get_val(("forge-{0}-{1}-installer.jar".format(version_game,
                                                                               forge_downloads_clint_install_version)),
                                         "sha1") == forge_downloads_clint_install_hash:
                            logger.info("重新下载完毕")
                            os.chdir(running_path)
                ...
                os.system('java -cp "{0};{1}" me.xfl03.HeadlessInstaller -installClient {2}'.format(
                    forge_install_headless_path, (os.path.join(game_path,
                                                               "forge-{0}-{1}-installer.jar".format(version_game,
                                                                                                    forge_downloads_clint_install_version))),
                    game_path))
        else:
